...
def train(args):
    ...
    if args.val_list is not None:
        ...
        val_loader = DataLoader(val_dataset, batch_size=512, shuffle=True, drop_last=True)
            ...
            if val_loader is not None:
                ...
                if avg_loss < best_val_loss:
                    ...
                    if args.path_results and osp.isdir(args.path_results):
                        model_path = osp.join(args.path_results, 'checkpoints', 'checkpoint_ronin_resnet_%d.pt' % epoch)
    ...
    if args.path_results:
        model_path = osp.join(args.path_results, 'checkpoints', 'checkpoint_ronin_resnet_latest.pt')
        torch.save({'model_state_dict': network.state_dict(),
...
def test_sequence(args):
    ...
    for data in args.test_list:
        ...
        ax3.set(xlabel=r'$p_n^x$ (m)', ylabel=r'$p_n^y$ (m)', title="Position of Resnet")
...
class HuskyResnetArgs():
    ...
    model_path = "../results/checkpoints/checkpoint_latest.pt"
    path_temp = "../temp"
    continue_from = None
    read_from_data = True
