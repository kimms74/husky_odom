...
if __name__ == '__main__':
    ...
    # datasets = ["square_ccw","square_cw","circle_ccw","circle_cw","ribbon","inf","random_2"]
        ...
        for n_iter, date_dir in enumerate(date_dirs):
            if (date_dir == dataset_name+"_mocap.txt"):
                ...
                for i in range(len(gt_back_left)):
                    ...
                    ratio = 0.29
                    p_gt_temp[i,0] = (gt_back_left[i,0]+(gt_front_left[i,0] - gt_back_left[i,0])*ratio + gt_back_right[i,0] + (gt_front_right[i,0] - gt_back_right[i,0])*ratio)/2
                    p_gt_temp[i,1] = (gt_back_left[i,1]+(gt_front_left[i,1] - gt_back_left[i,1])*ratio + gt_back_right[i,1] + (gt_front_right[i,1] - gt_back_right[i,1])*ratio)/2
                ...
                for i in range(len(imu_data)):
                    ...
                    acc_imu[i] = R_imu_from_mocap.dot(imu_data[i,22:25] - ((gravity - g_corr)/gravity)*gravity_vec[i])
                    acc_imu_temp[i,0:2] = imu_data[i,22:24] - (((gravity - g_corr)/gravity)*gravity_vec[i])[0:2]
                    # acc_imu_temp[i,0:2] = imu_data[i,22:24]
                    acc_imu[i] = R_imu_from_mocap.dot(acc_imu_temp[i])
