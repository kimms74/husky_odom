...
def launch(args):
    ...
    calc_odom(args, dataset)

class HUSKYDataset(BaseDataset):
    ...
    def __init__(self, args):
        super(HUSKYDataset, self).__init__(args)
    ...
    def read_data(args):
        """
        Read the data from the HUSKY dataset
        ...
        for n_iter, date_dir in enumerate(date_dirs):
            ...
            oxts = HUSKYDataset.load_oxts_packets_and_poses(oxts_files)
            ...
            if len(oxts) < HUSKYDataset.min_seq_dim:  #  sequence shorter than 30 s are rejected
            ...
            t = HUSKYDataset.load_timestamps(path2)
            ...
            t_tot += t[-1] - t[0]
            HUSKYDataset.dump(mondict, args.path_data_save, date_dir2)
    ...
    def pose_from_oxts_packet(packet, scale):
        ...
        Rx = HUSKYDataset.rotx(packet.roll)
        Ry = HUSKYDataset.roty(packet.pitch)
        Rz = HUSKYDataset.rotz(packet.yaw)
        R = Rz.dot(Ry.dot(Rx))
        ...
        for filename in oxts_files:
            with open(filename, 'r') as f:
                for line in f.readlines():
                    ...
                    packet = HUSKYDataset.OxtsPacket(*line)

                    if scale is None:
                        scale = np.cos(packet.lat * np.pi / 180.)

                    R, t = HUSKYDataset.pose_from_oxts_packet(packet, scale)

                    if origin is None:
                        origin = t

                    T_w_imu = HUSKYDataset.transform_from_rot_trans(R, t - origin)

                    oxts.append(HUSKYDataset.OxtsData(packet, T_w_imu))
        return oxts
...
def calc_odom(args, dataset):
    iekf = IEKF()
    # torch_iekf = TORCHIEKF()

    # put Kitti parameters
    iekf.filter_parameters = HUSKYParameters()
    iekf.set_param_attr()
...
class HUSKYArgs():
        ...
        # path_data_base = "../../Datasets/husky_dataset/fast_square_ccw"
        path_data_save = "../data"
        ...
        # test_filter = 1
        # results_filter = 1
        dataset_class = HUSKYDataset
        # parameter_class = HUSKYParameters
